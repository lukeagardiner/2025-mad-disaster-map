...
export default function Index() {
  ...
  // Location Fallback - Generic Attempt 2
  // If permissions denied... this will try to get a generic location 
  ...
  // Main Location Function to get precise location if available
  ...
  */
  ...
  useEffect(() => {
    const loadLocation = async () => {
      ...
      try {
        if (Platform.OS === 'web') {
          ...
          try {
            nativeLocation = await getCurrentLocation(); // First precise location attempt
          }
          catch (e) {
          ...
          if (nativeLocation) {
            ...
            try { 
              ...
              // Try getting a generic location from IP, then country, then use default - fallback logic
          ...
          // catch any naughty errors we've otherwise missed
    ...
    // load instruction is importanto
    loadLocation();
  }, []);

  /*
  ###################################################################
  ## -- VIEW CONTROL / CONTAINER LOGIC & DESIGN --                 ##
  ###################################################################
  */
  if (loading) { // View Loading Condition
    return (
  ...
  if (errorMsg) { // View Error Condition
    return (
      <View style={styles.wrapper}>
        <Text style={{ color: 'red', marginTop: 50, textAlign: 'center' }}>{errorMsg}</Text>
      </View>
    );
  }
  return ( // View Default Condition
    <View style={styles.wrapper}>
      {/*Settings Button*/}
      <Pressable
